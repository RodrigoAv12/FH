        ...
        <v-row>
          <v-col md="12" cols="12" justify-self="center" align-self="center">
            <v-row justify="center" >
              ...
              <v-col v-if="c > 1" class="d-flex" cols="12" md="6">
                <v-select :items="idiomas" label="Idioma" v-model="idioma_2"></v-select>
              </v-col>
              <v-col v-if="c > 1" class="d-flex" cols="12" md="6">
                <v-select :items="nivel" label="Nivel" v-model="idioma_nivel_2"></v-select>
              </v-col>
              <v-col v-if="c > 2" class="d-flex" cols="12" md="6">
                <v-select :items="idiomas" label="Idioma" v-model="idioma_3"></v-select>
              </v-col>
              <v-col v-if="c > 2" class="d-flex" cols="12" md="6">
                <v-select :items="nivel" label="Nivel" v-model="idioma_nivel_3"></v-select>
              </v-col>
            </v-row>
            <v-row align="center" justify="center">
              <v-col cols="12" md="12" justify-self="center">
                <v-btn v-show="c < 3" class="ma-2" color="teal" dark @click="c++">
                  <v-icon dark left>fas fa-plus</v-icon>Agregar
                </v-btn>
                <v-btn v-show="c > 1" class="ma-2" color="pink darken-1" dark @click="c--">
  ...
  methods:{
    agregarIdiomas() {
      var idiomas = {
        ...
        idioma_nivel_2: this.idioma_nivel_2,
        idioma_3: this.idioma_3,
        idioma_nivel_3: this.idioma_nivel_3,
